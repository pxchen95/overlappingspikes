...
for ii=length(x):-1:1
    h(ii+2*length(x))=plot([x(ii)-initbarwidth x(ii)+initbarwidth],[1 1]*(val(ii)+uppererror(ii)),'k-','linewidth',2);
    if ii==length(x); hold on; end;
    h(ii+length(x))=plot([x(ii)-initbarwidth x(ii)+initbarwidth],[1 1]*(val(ii)-lowererror(ii)),'k-','linewidth',2);
end;
...
for ii=length(x):-1:1
    h(ii)=plot([1 1]*x(ii),[val(ii)-lowererror(ii) val(ii)+uppererror(ii)],'k-','linewidth',2);
end;
