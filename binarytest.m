...
% plots error fractions vs SNRs, and displays row-normalized count matrix
% of truth vs. detection
...
%     if sum(count(:,1)) ~= 0, count(1,1) = count(1,1)/sum(count(:,1)); end % divide by # rejected
%     if sum(count(:,2)) ~= 0, count(1,2) = count(1,2)/sum(count(:,2)); end % divide by # detected
...
xlabel('SNR','fontsize',18); %ylabel('error fraction')
...
% print count matrix of truth x detection (currently prints final
% fractions)
...
H_z = 1/2*dot(y1,y1);
plot([H_z,H_z], [0,1], '--r')
text(H_z-0.12, 0.5, '\Psi_{opt}', 'rotation',90,'fontsize',18)
text(0,0.05,'\Psi < \Psi_{opt}', 'HorizontalAlignment','center','fontsize',18)
text(H_z*2,0.05,'\Psi > \Psi_{opt}', 'HorizontalAlignment','center','fontsize',18)
set(gca,'fontsize',18)
hold off
axis tight